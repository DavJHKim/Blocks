...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        for (int i = 0; i < piece.height(); i++) {
            for (int j = 0; j < piece.width(); j++) {
                if (get(row + i, col + j) && piece.get(i, j)) {
                    return false;
                }
            }
        }
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        if (nrows > 0 || ncols > 0) {
            _streakLength += 1;
            return _streakLength * ((nrows * _width) + (ncols * _height));
        } else {
            _streakLength = 0;
            return (nrows * _width) + (ncols * _height);
        }
    }
    ...
    void undo() {
        if (_current > 0) {
            _history.get(_current - 1).restoreState();
            _current = _current - 1;
        }
    }
